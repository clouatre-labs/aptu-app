...
## Rust FFI and Kotlin Multiplatform
...
- Verify that UniFFI bindings in `crates/aptu-ffi/` correctly expose types and methods from
  `aptu-core` without unnecessary wrapper types.
- When reviewing Android/iOS Kotlin code, ensure that FFI calls are properly wrapped with
  error handling and that null safety is respected.
